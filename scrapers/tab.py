...
class TabScraper(Scraper):
    def get_bookie_type(self):
        return BookieType.TAB

    def setup(self):
        self.driver.find_element(by=By.XPATH,
                                 value='.//button[@data-testid="header-login"]').click()

        try:
            login_elem = WebDriverWait(self.driver, self.TIMEOUT).until(
                EC.presence_of_element_located(
                    (By.XPATH, '//div[@data-testid="login-modal"]')))
        ...
        (login_elem
         .find_element(by=By.XPATH,
                       value='.//input[@data-testid="account-number-input"]')
         .send_keys(os.environ['TAB_ACCOUNT_NUMBER'])
         )
        (login_elem
         .find_element(by=By.XPATH,
                       value='.//input[@data-testid="password-input"]')
    ...
    def loop(self):
        try:
            runner_elems = WebDriverWait(self.driver, self.TIMEOUT).until(
                EC.visibility_of_all_elements_located((By.XPATH,
                                                       '//div[@class="pseudo-body"]/div[@class="row"]')))
        except TimeoutException:
            print(f'Loading {self.get_bookie_type()} took too much time!')
            self.stop()
            return
        ...
        for runner_elem in runner_elems:
            name_elem = (runner_elem
                         .find_element(by=By.XPATH,
                                       value='.//div[@class="runner-name"]'))
            ignore_len = sum([len(child_elem.text) for child_elem in
                              name_elem.find_elements(by=By.XPATH,
                                                      value='./*')])
            name = name_elem.text
            if ignore_len > 0:
