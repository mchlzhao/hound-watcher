...
class PalmerbetScraper(Scraper):
    def get_bookie_type(self):
        return BookieType.PALMERBET

    def loop(self):
        try:
            runner_elems = WebDriverWait(self.driver, self.TIMEOUT).until(
                EC.visibility_of_all_elements_located((By.XPATH,
                                                       '//tr/app-race-runner/div[not(contains(@class, "scratched"))]')))
        ...
        for runner_elem in runner_elems:
            name_elem = (runner_elem
                         .find_element(by=By.XPATH,
                                       value='.//div[contains(@class, "runner-title")]'))
            ...
            back_odds_elems = (runner_elem
                               .find_elements(by=By.XPATH,
                                              value='.//td[not(contains(@class, "place"))]//app-price-display/span'))
