...
def bonus_back_if_place_but_no_win(top_n, odds, prices):
    ...
    betfair_n_places = prices.get(f'betfair_{top_n}_places')
    if any(map(lambda x: x is None, [odds, betfair_win, betfair_n_places])):
        return None
    if odds.back_odds is None or betfair_win.mid_prob is None or \
            betfair_n_places.mid_prob is None:
        return None
    ...
    ev_per_dollar = ev_given_probs(betfair_win.mid_prob,
                                   betfair_n_places.mid_prob)
    bounds = (ev_given_probs(betfair_win.lay_prob, betfair_n_places.lay_prob),
              ev_given_probs(betfair_win.back_prob, betfair_n_places.back_prob))
