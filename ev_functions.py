...
def bonus_back_if_place_but_no_win(top_n, odds, prices):
    ...
    def ev_given_probs(win_prob, place_prob):
        return odds.back_odds * win_prob + BONUS_TO_CASH_RATIO * (place_prob - win_prob) - 1

    ev_per_dollar = ev_given_probs(betfair_win.mid_prob, betfair_n_place.mid_prob)
...
def double_winnings_in_bonus(odds, prices):
    ...
    def ev_given_prob(prob):
        return (odds.back_odds + (odds.back_odds - 1) * BONUS_TO_CASH_RATIO) * prob - 1

    ev_per_dollar = ev_given_prob(betfair_win.mid_prob)
    bounds = (ev_given_prob(betfair_win.lay_prob), ev_given_prob(betfair_win.back_prob))
    return ((ev_per_dollar, *bounds), ev_per_dollar * BET_SIZE / (odds.back_odds - 1))
